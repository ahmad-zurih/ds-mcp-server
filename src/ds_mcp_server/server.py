...
def patch_file(file_path: str, old_str: str, new_str: str) -> str:
    """
    Find the FIRST (and only) occurrence of old_str in a file and replace it with new_str.
    PREFER this over write_file for edits to existing files — only send the changed part.
    old_str must match EXACTLY, including whitespace and indentation. Make it specific enough that it is unique in the file.
